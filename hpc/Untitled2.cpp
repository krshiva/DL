...
#pragma omp parallel for private(i, temp)
    ...
    void parallel_merge_sort(int arr[], int l, int r,int threads) {
        if (l < r) {
            int m = l + (r - l) / 2;
            if(threads>1 && r-l+1>=threads*100){
    ...
    void test_bubble_sort_performance(int n) {
        ...
        // Measure the execution time of odd_even_parallel bubble sort
        start_time = chrono::high_resolution_clock::now();
        parallel_bubble_sort_odd_even(arr_copy, n);
        end_time = chrono::high_resolution_clock::now();
        auto odd_even_parallel_sort_time = chrono::duration_cast<chrono::microseconds>(end_time - start_time).count();
        ...
        cout << "Odd Even Parallel bubble sort time: " << odd_even_parallel_sort_time << " microseconds" << endl;
